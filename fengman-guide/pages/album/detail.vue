	...
	<view class="album-detail-page">
		...
		<scroll-view :scroll-y="true" class="photo-scroll-view">
			<view class="content-wrapper" style="padding-bottom: 160rpx">
				...
				<view class="photo-grid">
					<view v-for="(photo, index) in filteredPhotos" :key="photo._id" class="photo-item" @click="handlePhotoTap(photo, index)" @longpress="handleLongPress(photo)">
		...
		<view class="batch-bar" v-if="isSelectionMode">
			...
			<view class="batch-actions">
				...
				<view class="action-item" @click="handleBatchShare">
					<view class="icon-box">
						<uni-icons type="paperplane" size="24" color="#333"></uni-icons>
					</view>
					<text>分享</text>
				</view>
...
export default {
	...
	methods: {
		...
		handleUpload() {
			...
			uni.showModal({
				title: '上传重要提示',
				content: '为了保证照片清晰度，请在接下来的相册界面底部，务必勾选【原图】按钮！',
				confirmText: '去选择',
				showCancel: true, // 允许取消，万一点错了
				cancelText: '取消',
				success: (res) => {
					if (res.confirm) {
						// 用户点击“去选择”，才打开系统相册
						startChooseAndUpload();
					}
				}
			});
		},
		...
		handlePhotoTap(photo, index) {
			if (this.isSelectionMode) {
				// 多选模式：切换选中状态
				this.toggleSelection(photo);
			} else {
				// 普通模式：预览大图
				this.previewMediaItem(photo, index);
			}
		},

		handleLongPress(photo) {
			// 如果已经是多选模式，不做处理（或者可以视为普通选中）
			...
			this.selectedPhotoIds = [photo._id]; // 默认选中长按的那一张
		},
		...
		handleBatchDelete() {
			...
			const count = this.selectedPhotoIds.length;
			...
			uni.showModal({
				...
				success: async (res) => {
					if (res.confirm) {
						...
						try {
							// 循环删除 (建议后端最好提供 batchDelete 接口，这里演示前端循环调用)
							// 注意：这里需要考虑权限，如果选了别人的照片，接口可能会报错
							const deletePromises = this.selectedPhotoIds.map((id) => {
								// 调用原有的删除API
								return albumService.deletePhoto(id).catch((err) => {
									console.error(`删除 ${id} 失败`, err);
									return { errCode: 1 }; // 标记失败
								});
							});
							...
							uni.hideLoading();
							uni.showToast({ title: '操作结束', icon: 'none' });
		...
		async handleBatchDownload() {
			...
			// 2. 循环下载（为了避免瞬间内存溢出，建议使用 for...of 串行或分组处理）
			...
			uni.showModal({
				title: '下载完成',
				content: `成功 ${successCount} 张${failCount > 0 ? '，失败 ' + failCount + ' 张' : ''}，已保存到手机相册。`,
				showCancel: false,
				success: () => {
					this.exitSelectionMode();
				}
		...
		handleBatchShare() {
			...
			// 限制：如果只选了1张，调用原生分享
			if (this.selectedPhotoIds.length === 1) {
				const photo = this.filteredPhotos.find((p) => p._id === this.selectedPhotoIds[0]);
				this.shareSinglePhoto(photo); // 复用之前的单张分享逻辑
			} else {
				// 如果选了多张，提示不支持
				uni.showModal({
					title: '提示',
					content: '微信暂不支持同时分享多张图片。\n您可以先点击“下载”保存到相册，然后去微信聊天中发送相片。',
					confirmText: '去下载',
					success: (res) => {
						if (res.confirm) {
...
.selection-overlay {
	...
	width: 100%;
	height: 100%;
	background-color: rgba(0, 0, 0, 0.1); /* 未选中时轻微变暗 */
	z-index: 10;
	transition: all 0.2s;
}
