		...
		<view class="content-area" v-else-if="hasItinerary">
			...
			<view class="itinerary-header">
				...
				<view class="my-4 flex items-center gap-2">
					...
					<view class="theme-card bg-purple-50 ml-2" @click="manageSupplies" hover-class="theme-card-active">
						<view class="theme-icon-wrapper bg-purple-100">
							<uni-icons type="list" size="20" color="#9333ea"></uni-icons>
						</view>
						<view class="theme-card-title">
							<text>物资管理</text>
						</view>
...
export default {
	...
	methods: {
			...
			for (const staff of staves) {
				if (staff.role && Array.isArray(staff.role)) {
					...
					if (staff.role.includes('guide')) {
						...
						console.log('[解析Staves] 找到私导电话:', staff.mobile);
					}
					// 查找管家
		...
		// 物资管理
		manageSupplies() {
			// 跳转到你即将开发的物资管理页，传递订单ID
			uni.navigateTo({
				url: `/pages/order/order-supplies?orderId=${this.currentOrder.order_id}`
			});
		},
		...
		getCompressedImageUrl(url) {
			...
			// 1. 预处理：判断是否为携程系域名
			// 携程图片可能带有 encoded 的字符，所以先 decode 方便正则匹配，最后再 encode 输出
			const rawUrl = decodeURI(url);
			const isCtrip = rawUrl.includes('ctrip.com') || rawUrl.includes('trip.com');
			const isAliyun = rawUrl.includes('bspapp.com') || rawUrl.includes('aliyuncs.com');

			// 2. 携程/去哪儿/Trip.com 图片特殊处理 (优先处理)
			if (isCtrip) {
				// 正则解释：
				// _       : 以不划线开头
				...
				return this.getEncodedUrl(cleanUrl + '_C_800_800_Q80.jpg');
			}
			...
			if (encodedUrl.includes('x-oss-process') || url.includes('_R_') || url.includes('_C_')) {
				return encodedUrl;
			}
