<template>
	<view class="min-h-screen bg-white flex flex-col">
		...
		<view class="flex-1 flex flex-col justify-center items-center p-5">
			...
			<text class="skip-login-text">账号仅限内部人员登录使用</text>
		</view>
	</view>
</template>
	...
	data() {
		return {
			...
			appConfig: {
				...
				title: '风漫旅行管家',
				title_en: 'F M G U I D E',
				slogan: '随风漫行 × 向心而生',
				subslogan: '服务 | 引领 | 真诚 | 陪伴',
				description: '专业旅行私导服务，不负每一段旅途\nEVERY JOURNEY COUNTS'
			},
			redirectType: '',
			targetOrderId: ''
		};
	...
	methods: {
		...
		handleLoginSuccess(result) {
			...
			setTimeout(async () => {
				...
				// 2. 核心身份判断逻辑
				let role = [];
				const roleRes = await db.collection('uni-id-users').where('_id == $cloudEnv_uid').field('role').get();
				if (roleRes.result && roleRes.result.data && roleRes.result.data.length > 0) {
					role = roleRes.result.data[0].role || [];
				}

				// 检查是否包含 'guide' 角色
				if (role.includes('guide')) {
					// 已经是司导 -> 正常进入首页
					console.log('身份验证通过：司导');
					uni.reLaunch({
						url: '/pages/home/home'
					});
				} else {
					...
					try {
						const profileRes = await db
							...
							.field('personality_type, status') // 获取关键字段
							.limit(1)
							.get();
						...
						if (profileRes.result.data.length > 0) {
							...
							// 逻辑分支 1：资料已提交，但未做测试 (无 personality_type)
							if (!profile.personality_type) {
								console.log('资料已提交，前往性格测试');
								uni.redirectTo({
									url: '/pages/register/quiz'
								});
							}
							// 逻辑分支 2：资料已提交且测试已做 -> 审核中/等待
							else {
								console.log('流程已完成，跳转审核页');
								uni.redirectTo({
							...
							// 逻辑分支 3：无资料 -> 注册页
							console.log('未提交资料，前往注册');
							uni.redirectTo({
								url: '/pages/register/register'
							});
						}
					} catch (e) {
						console.error('查询资料状态失败:', e);
						// 如果查询出错（如断网），为了不卡死用户，通常兜底跳转到注册页或显示错误
						uni.redirectTo({
							url: '/pages/register/register'
						});
					}
