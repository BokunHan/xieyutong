    ...
    <view class="max-w-7xl mx-auto px-8 py-8 space-y-8">
        ...
        <view class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
          <view class="flex items-center justify-between">
            <view>
              ...
              <text class="text-gray-600 text-lg block mt-1">活跃券数</text>
            </view>
      ...
      <view class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <unicloud-db 
          ...
          field="title,description,amount,min_amount,issue_type,total_count,used_count,valid_days,share_code,auto_issue_condition,status,created_at,updated_at" 
          :where="where" 
          page-data="replace"
          :orderby="orderby" 
          :getcount="true" 
                   ...
                   <td class="px-6 py-6 text-center border-r border-gray-100">
                     <view class="flex flex-col items-center space-y-2">
                       <view class="bg-red-50 p-3 rounded-xl border border-red-200">
                         <text class="text-3xl font-bold text-red-600 block">￥{{ item.amount }}</text>
                       </view>
                       <view class="text-center space-y-1">
                         <text class="text-gray-700 text-sm font-medium block">
                           满 <span class="text-orange-600 font-bold">￥{{ item.min_amount }}</span> 可用
                         </text>
                   ...
                   <td class="px-6 py-6 text-center border-r border-gray-100">
                     <view class="space-y-3">
                       <span :class="getIssueTypeClass(item.issue_type)" 
                             class="inline-block px-3 py-2 rounded-lg text-sm font-bold border-2">
                   ...
                   <td class="px-6 py-6 text-center border-r border-gray-100">
                     <view class="flex justify-center">
                       <span :class="getStatusClass(item.status)" 
                             class="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold border-2 min-w-[80px] justify-center">
          ...
          <view v-if="data.length > 0" class="bg-gray-50 px-8 py-6 border-t border-gray-200">
            <view class="flex items-center justify-between">
              <text class="text-gray-600 text-lg">
                ...
                {{ Math.min(pagination.current * pagination.size, pagination.count) }} 条，
  ...
  methods: {
    ...
    async loadStatistics() {
      try {
        ...
        this.todayIssued = coupons.filter(item => 
    ...
    delTable() {
      ...
      uni.showModal({
        title: '确认删除',
        content: `确定要删除选中的 ${this.selectedIndexs.length} 条记录吗？`,
        success: (res) => {
          if (res.confirm) {
