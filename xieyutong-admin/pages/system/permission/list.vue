	...
	<view class="fix-top-window">
		...
		<view class="uni-container">
			<unicloud-db
				...
				field="permission_id,permission_name,permission_desc,create_date"
				:where="where"
				page-data="replace"
				:orderby="orderby"
				:getcount="true"
				...
				<uni-table ref="table" :loading="loading" :emptyText="error.message || $t('common.empty')" border stripe type="selection" @selection-change="selectionChange">
					<uni-tr>
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'permission_id')" sortable @sort-change="sortChange($event, 'permission_id')">
							权限标识
						</uni-th>
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'permission_name')" sortable @sort-change="sortChange($event, 'permission_name')">
							权限名称
						</uni-th>
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'permission_desc')" sortable @sort-change="sortChange($event, 'permission_desc')">
							权限描述
						</uni-th>
					...
					<uni-tr v-for="(item, index) in data" :key="index">
						...
						<uni-td align="center">{{ item.permission_desc }}</uni-td>
						<uni-td align="center">
...
<script>
import { enumConverter, filterToWhere } from '@/js_sdk/validator/uni-id-permissions.js';
