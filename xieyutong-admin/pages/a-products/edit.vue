  ...
  export default {
    ...
    methods: {
      ...
      async getPolicyData(productId) {
        ...
        try {
          const res = await db.collection('a-booking-policies')
      ...
      async getReviewData(productId) {
        ...
        try {
          const res = await db.collection('a-reviews')
            .where(`product_id == "${productId}"`)
      ...
      async saveItineraryData(data) {
        ...
        try {
          ...
          // 检查是否已存在行程记录
          const existQuery = await db.collection('a-itineraries')
