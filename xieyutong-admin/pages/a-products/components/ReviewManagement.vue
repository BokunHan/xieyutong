    ...
    <view class="bg-gradient-to-r from-purple-500 to-purple-600 px-8 py-6">
      <view class="flex items-center justify-between">
        ...
        <view class="flex items-center space-x-3">
          <button @click="generateAIReviews" 
                  class="px-6 py-2 bg-white text-purple-600 rounded-lg hover:bg-gray-50 transition-colors font-medium">
            <i class="fas fa-robot mr-2"></i>
            生成AI评价
          </button>
          <button @click="addManualReview" 
                  class="px-6 py-2 bg-white text-purple-600 rounded-lg hover:bg-gray-50 transition-colors font-medium">
    ...
    <view class="overflow-hidden">
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-100">
          <tr>
            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider w-32">
              用户信息
            </th>
            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider w-24">
              评分
            </th>
            ...
            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider w-40">
              创建时间
            </th>
            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider w-32">
              操作
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="(review, index) in paginatedReviews" :key="review.review_id || index"
            ...
            <td class="px-6 py-4">
              <view class="max-w-xs">
                ...
                <view v-if="review.images && review.images.length > 0" class="mt-2 flex space-x-2">
                  <view v-for="(image, imgIndex) in review.images.slice(0, 3)" :key="imgIndex"
                        class="w-8 h-8 bg-gray-200 rounded border">
                    <image :src="image" class="w-full h-full object-cover rounded" mode="aspectFill"/>
                  </view>
                  <text v-if="review.images.length > 3" class="text-xs text-gray-500 flex items-center">
            ...
            <td class="px-6 py-4">
              <span :class="[
                ...
                review.is_ai ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
              ]">
            ...
            <td class="px-6 py-4">
              <select 
                ...
                :class="[
                  ...
                  review.status === 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'
                ]"
              >
            ...
            <td class="px-6 py-4">
              <view class="flex items-center space-x-2">
                ...
                <button @click="deleteReview(review, index)" 
          ...
          <view class="flex items-center space-x-2">
            <button @click="changePage(currentPage - 1)" 
                    ...
                    :class="[
                      ...
                      currentPage <= 1 ? 'bg-gray-100 text-gray-400' : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50'
                    ]">
              上一页
            </button>
            ...
            <button @click="changePage(currentPage + 1)" 
                    ...
                    :class="[
                      'px-3 py-1 rounded text-sm',
                      currentPage >= totalPages ? 'bg-gray-100 text-gray-400' : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50'
                    ]">
              下一页
            </button>
          </view>
      ...
      <view class="bg-white rounded-lg p-8 w-full max-w-2xl mx-4" @click.stop>
        ...
        <view class="space-y-6">
          ...
          <view>
            ...
            <uni-easyinput 
              v-model="editingReview.user_name"
...
export default {
  ...
  methods: {
    ...
    generateMockAIReviews(count) {
      ...
      const reviews = [];
      const names = ['王先生', '李女士', '张同学', '刘小姐', '陈先生', '赵女士', '孙同学', '周小姐'];
      const contents = [
    ...
    updateReviewStatus(review) {
      ...
      console.log('🔄 [ReviewManagement] updateReviewStatus() - 状态含义:', 
        review.status === 0 ? '待审核' : 
        review.status === 1 ? '已发布' : 
        review.status === 2 ? '已隐藏' : '未知状态'
      );
