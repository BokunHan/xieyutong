  ...
  <view>
    ...
    <view class="mx-6 my-6">
      <unicloud-db 
        ...
        field="user_id,coupon_id,coupon_code,status,order_id,source_type,source_detail,received_at,used_at,expired_at,amount,min_amount,title,remark,created_at,updated_at" 
        :where="where" 
        page-data="replace"
        :orderby="orderby" 
        :getcount="true" 
          ...
          <view class="p-4 border-b border-gray-200">
            <view class="flex items-center justify-between">
              ...
              <view class="text-sm text-gray-500">
                共找到 <text class="font-semibold text-amber-600 text-base">{{pagination.count}}</text> 张优惠券
              </view>
            </view>
          </view>
              ...
              <table class="w-full" style="min-width: 1800px;">
                ...
                <thead class="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th class="px-4 py-4 text-center text-sm font-semibold text-gray-900 w-32">
                      操作
                    </th>
                  </tr>
                </thead>
                  ...
                  >
                    ...
                    <td class="px-4 py-4">
                      <view class="flex items-center">
                        ...
                        <text class="text-sm font-medium text-gray-900 font-mono cursor-pointer hover:text-amber-600 transition-colors" @click="copyToClipboard(item.user_id)">
                          {{item.user_id}}
                        </text>
                      </view>
                    </td>
                    ...
                    <td class="px-4 py-4">
                      <text class="text-sm text-gray-600">
                        {{item.min_amount ? '满¥' + formatAmount(item.min_amount) : '无门槛'}}
                      </text>
                    </td>
                    ...
                    <!-- 关联订单 -->
                    <td class="px-4 py-4">
                      <text v-if="item.order_id" class="text-sm text-blue-600 hover:text-blue-800 cursor-pointer transition-colors font-mono" @click="copyToClipboard(item.order_id)">
                        {{item.order_id}}
                      </text>
...
const dbSearchFields = ['coupon_code', 'title', 'remark'] // 模糊搜索字段
const pageSize = 15
  const pageCurrent = 1
  ...
  export default {
    ...
    methods: {
      ...
      delTable() {
        ...
        uni.showModal({
          title: '确认删除',
          content: `确定要删除选中的 ${this.selectedIndexs.length} 张优惠券吗？此操作不可恢复。`,
          success: (res) => {
            if (res.confirm) {
