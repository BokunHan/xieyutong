<template>
  <view>
    ...
    <view class="uni-container">
      <unicloud-db ref="udb" :collection="collectionList" field="user_id,level,total_consumption,order_count,upgrade_threshold,benefits,status,join_date,upgrade_date,expire_date,created_at,updated_at" :where="where" page-data="replace"
        ...
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection" @selection-change="selectionChange">
          <uni-tr>
            ...
            <uni-th align="center" sortable @sort-change="sortChange($event, 'upgrade_threshold')">升级条件</uni-th>
            <uni-th align="center" sortable @sort-change="sortChange($event, 'benefits')">会员权益</uni-th>
            ...
            <uni-th align="center" filter-type="timestamp" @filter-change="filterChange($event, 'join_date')" sortable @sort-change="sortChange($event, 'join_date')">join_date</uni-th>
            <uni-th align="center" filter-type="timestamp" @filter-change="filterChange($event, 'upgrade_date')" sortable @sort-change="sortChange($event, 'upgrade_date')">upgrade_date</uni-th>
            <uni-th align="center" filter-type="timestamp" @filter-change="filterChange($event, 'expire_date')" sortable @sort-change="sortChange($event, 'expire_date')">expire_date</uni-th>
          ...
          <uni-tr v-for="(item,index) in data" :key="index">
            ...
            <uni-td align="center">{{options.level_valuetotext[item.level]}}</uni-td>
            <uni-td align="center">{{item.total_consumption}}</uni-td>
            <uni-td align="center">{{item.order_count}}</uni-td>
            <uni-td align="center">{{item.upgrade_threshold}}</uni-td>
            <uni-td align="center">{{item.benefits}}</uni-td>
            ...
            <uni-td align="center">
              <uni-dateformat :threshold="[0, 0]" :date="item.upgrade_date"></uni-dateformat>
            </uni-td>
            <uni-td align="center">
