<template>
	<div class="itinerary-management">
		<!-- 页面标题和操作区 -->
		<div class="itinerary-sidebar" v-if="editableData.itinerary && editableData.itinerary.length > 0">
			<div class="sidebar-title">行程导航</div>
			<div class="sidebar-list">
				<div
					v-for="(day, index) in editableData.itinerary"
					:key="day.day"
					class="sidebar-item"
					:class="{ active: currentScrollIndex === index, dragging: draggingIndex === index }"
					draggable="true"
					...
					@contextmenu.prevent="onSidebarRightClick($event, index)">
					<div class="sidebar-item-handle">
						<i class="fas fa-grip-vertical"></i>
					</div>
					<span class="sidebar-item-text">D{{ day.day }}</span>
					<span class="sidebar-item-tooltip">{{ day.destination_city || '第' + day.day + '天' }}</span>
				</div>
			</div>
		</div>

		<div v-if="contextMenu.visible" class="sidebar-context-menu" :style="{ top: contextMenu.top + 'px', left: contextMenu.left + 'px' }" @click.stop>
			<div class="context-menu-item" @click="insertDayAfter(contextMenu.index)">
				<i class="fas fa-plus-circle text-blue-500"></i>
				<span>在下方插入一天</span>
			</div>
			<div class="context-menu-divider"></div>
			<div class="context-menu-item delete" @click="confirmDeleteDay(contextMenu.index)">
		...
		<scroll-view scroll-y scroll-with-animation class="main-content-scroll" :scroll-into-view="scrollTarget" @scroll="onScroll">
...
<script>
const rpaObj = uniCloud.importObject('a-task-rpa');
...
export default {
	...
	data() {
		return {
			draggingIndex: null, // 当前被拖拽的索引
			dragTargetIndex: null, // 拖拽目标索引
			currentScrollIndex: null, // 当前滚动到的天数索引
			scrollTarget: '',
			scrollTimer: null,
	...
	methods: {
		...
		async handleDrop(targetIndex) {
			// 如果位置没变，直接返回
			if (this.draggingIndex === targetIndex || this.draggingIndex === null) {
			...
			const itinerary = this.editableData.itinerary;
			const movedItem = itinerary[this.draggingIndex];

			// 1. 在数组中移动元素
			// 先移除
			itinerary.splice(this.draggingIndex, 1);
			// 再插入到新位置
			itinerary.splice(targetIndex, 0, movedItem);

			// 2. 重新计算所有天数的 day 字段 (1, 2, 3...)
			itinerary.forEach((day, index) => {
				day.day = index + 1;
				// 可选：如果标题是默认格式 "第X天"，也顺便更新标题，防止标题错乱
				if (/^第\d+天$/.test(day.day_title)) {
					day.day_title = `第${day.day}天`;
				}
			});

			// 3. 更新总天数
			this.editableData.total_days = itinerary.length;

			// 4. 发送数据更新
			this.$emit('update-partial', { path: 'itinerary', value: itinerary });

			console.log('fromIndex: ', fromIndex, ' toIndex: ', toIndex, ' this.editableData.order_id: ', this.editableData.order_id);
			if (this.editableData.order_id) {
				rpaObj
					.handleItineraryChange({
						orderId: this.editableData.order_id,
						action: 'move',
						data: {
							fromIndex: fromIndex,
							toIndex: toIndex
					...
					.then((res) => {
						console.log('任务顺序已同步', res);
					})
					.catch((err) => {
						console.error('任务顺序同步失败', err);
					});
			}
		...
		// 监听滚动事件 (添加防抖/节流)
		onScroll(e) {
			// 如果正在进行点击跳转（通过判断 scrollTarget 是否有值），则暂停监听，避免闪烁
			if (this.scrollTarget) return;

			// 简单的节流阀：每 100ms 计算一次，避免性能消耗过大
			if (this.scrollTimer) return;

			this.scrollTimer = setTimeout(() => {
				this.checkVisibleDay();
		...
		// 计算当前可见的日期
		checkVisibleDay() {
			// 获取所有 class 为 day-container 的元素位置
			const query = uni.createSelectorQuery().in(this);
			query
				.selectAll('.day-container')
				.boundingClientRect((rects) => {
					if (!rects || rects.length === 0) return;

					// 设定一个触发线（阈值），比如距离顶部 150px 的位置
					// 这个值通常是 顶部Header高度 + 侧边栏的起始偏移量
					const triggerLine = 150;

					let activeIndex = -1;

					// 遍历所有天数容器
					// rect.top 是元素顶部距离可视区域顶部的距离
					for (let i = 0; i < rects.length; i++) {
						// 如果该元素的顶部 已经“越过”了 触发线（即滚上去了），或者是第一天
						if (rects[i].top < triggerLine) {
							activeIndex = i;
						} else {
							// 因为是按顺序排列的，一旦发现某个元素还在触发线下面，后面的肯定也在下面，直接停止循环
							break;
						}
					}

					// 如果找到匹配的索引，更新 currentScrollIndex
					if (activeIndex !== -1) {
						// 防止重复赋值
						if (this.currentScrollIndex !== activeIndex) {
							this.currentScrollIndex = activeIndex;
						}
					} else {
						// 如果所有元素都在下面（比如刚开始加载），默认高亮第一天
			...
			try {
				...
				if (this.editType === 'product') {
					...
					result = await db
						...
						.doc(this.snapshotId)
						.field('itinerary, title, sub_title, total_days, remarks, product_id, ctrip_id, _id, order_id') // <--- 只获取需要的字段
						.get();
				}
		...
		deleteDay(dayIndex) {
			...
			uni.showModal({
				...
				success: (res) => {
					if (res.confirm) {
						const oldTotalDays = this.editableData.itinerary.length;
						...
						if (this.editableData.order_id) {
							uni.showLoading({ title: '同步任务中...' });
							try {
								rpaObj.handleItineraryChange({
									orderId: this.editableData.order_id,
									action: 'delete',
									data: { dayIndex: dayIndex, totalDaysBefore: oldTotalDays }
								});
							} catch (e) {
								console.error('任务同步失败', e);
							} finally {
								uni.hideLoading();
							}
						}
		...
		// 1. 右键点击事件
		onSidebarRightClick(event, index) {
			// 计算菜单位置，避免溢出屏幕
			let top = event.clientY;
			let left = event.clientX + 10; // 稍微向右偏移
		...
		insertDayAfter(index) {
			...
			console.log(`➕ 在第${index + 1}天后插入新的一天`);
			...
			const oldTotalDays = this.editableData.itinerary.length;
			console.log('oldTotalDays: ', oldTotalDays);
			...
			// 插入数组
			this.editableData.itinerary.splice(insertIndex, 0, newDay);
			...
			if (this.editableData.order_id) {
				// 同样静默处理，或者显示简短提示
				const rpaObj = uniCloud.importObject('a-task-rpa');
				// 注意：这里传的是 insertIndex (即 index + 1)，因为这是新的一天实际所在的索引
				rpaObj
					.handleItineraryChange({
						orderId: this.editableData.order_id,
						action: 'insert',
						data: { insertIndex: insertIndex, totalDaysBefore: oldTotalDays }
					})
					.then((res) => {
						console.log('任务已顺延', res);
					})
					.catch((err) => {
						console.error('任务顺延失败', err);
					});
			}

			// 滚动到新创建的这一天
			this.$nextTick(() => {
				this.scrollToDay(insertIndex);
				this.saveField('itinerary'); // 触发更新
		...
		// 辅助方法：重排所有天数序号
		reorderDays() {
			this.editableData.itinerary.forEach((day, idx) => {
				day.day = idx + 1;
				if (/^第\d+天$/.test(day.day_title)) {
					day.day_title = `第${day.day}天`;
				}
...
.sidebar-item.dragging .sidebar-item-tooltip {
	display: none !important;
}

/* 拖拽把手图标（默认隐藏，hover显示或仅用于指示） */
.sidebar-item-handle {
	display: none;
}
...
@media (max-width: 1024px) {
	.itinerary-management {
		padding-left: 20px; /* 小屏幕取消左侧padding */
	}
	.itinerary-sidebar {
		display: none; /* 小屏幕隐藏侧边栏，避免遮挡 */
	}
}
