	...
	<view>
		...
		<view class="mx-6 my-6">
			<unicloud-db
				...
				field="_id,order_no,user_id{mobile},rank,product_snapshot,status,quantity,final_amount,contact_info,departure_date,duration_days,created_at,travel_users,staves"
				:where="where"
				page-data="replace"
				:orderby="orderby"
				:getcount="true"
							...
							<table class="w-full">
								<thead class="bg-gray-50 border-b border-gray-200">
									<tr>
										...
										<th class="px-4 py-4 text-left text-sm font-semibold text-gray-900 w-15">
											等级
											<i class="fas fa-sort text-gray-400 ml-1 cursor-pointer" @click="sortChange({ order: 'ascending' }, 'rank')"></i>
										</th>
										...
										<th class="px-4 py-4 text-left text-sm font-semibold text-gray-900 w-32">出行用户</th>
										<th class="px-4 py-4 text-left text-sm font-semibold text-gray-900 w-32">服务人员</th>
					...
					<view class="p-6">
						<uni-forms v-if="currentEditId" ref="editForm" :modelValue="formData" :rules="formRules" validate-trigger="bind" label-width="100" label-align="right">
							...
							<uni-forms-item name="rank" label="订单等级">
								<uni-data-checkbox v-model="formData.rank" :localdata="rankOptions" mode="tag"></uni-data-checkbox>
							</uni-forms-item>
