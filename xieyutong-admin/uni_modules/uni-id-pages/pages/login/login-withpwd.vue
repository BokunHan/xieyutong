...
<template>
	<view class="uni-content">
		...
		<view class="link-box">
			...
			<text v-if="!config.isAdmin" class="link" @click="toRegister">{{ !existAdmin ? '注册管理员账号' : '注册账号' }}</text>
		</view>
		<!-- 悬浮登录方式组件 -->
		<!-- #ifndef MP-TOUTIAO -->
...
export default {
	...
	async onLoad() {
		...
		try {
			...
			// 	.where({ role: dbCmd.in(['admin', 'super_admin']) })
			// 	.count();
			// let count = countRes.result.total;
			// this.existAdmin = count > 0 ? true : false;
			this.existAdmin = await utils.checkAdminExists();
		} catch (err) {
			console.error('checkAdminExists 调用失败:', err);
	...
	methods: {
		...
		toRegister() {
			uni.navigateTo({
				url: !this.existAdmin ? '/uni_modules/uni-id-pages/pages/register/register-admin' : '/uni_modules/uni-id-pages/pages/register/register',
				fail(e) {
					console.error(e);
				}
			});
