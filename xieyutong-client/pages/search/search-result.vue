	...
	<view class="min-h-screen bg-gray-50">
		...
		<view class="bg-white border-b border-gray-100" :style="{ height: navBarHeight + 'px' }">
			<view class="flex items-center h-full px-3" :style="{ paddingTop: menuButtonInfo.top - statusBarHeight + 'px' }">
				<!-- 返回按钮 -->
				<view class="flex items-center justify-center" :style="{ width: menuButtonInfo.height + 'px', height: menuButtonInfo.height + 'px' }" @click="goBack">
					<text class="fa fa-arrow-left text-gray-800 text-lg"></text>
				</view>

				<!-- 搜索框 -->
				<view class="flex-1 mx-2" @click="goToSearch" :style="{ marginRight: menuButtonInfo.width + 12 + 'px' }">
		...
		<scroll-view
			...
			<view v-else-if="productList.length > 0" class="px-3">
				<view v-for="(product, index) in productList" :key="product.id || index" class="bg-white mb-3 rounded-lg overflow-hidden shadow-sm" @click="goToProductDetail(product.id)">
					<image :src="getOptimizedImage(product.image, 400, 400)" class="w-full h-48 object-cover" mode="aspectFill"></image>
					<view class="p-3">
				...
				<view class="py-4 text-center" v-if="hasMore">
					...
					<view v-else class="text-blue-500 text-sm py-2" @click="loadMoreResults">点击加载更多</view>
				</view>

				<!-- 没有更多数据 -->
				<view v-else class="py-4 text-center text-xs text-gray-400">已显示全部搜索结果</view>
			</view>
			...
			<view v-else class="flex flex-col items-center justify-center py-20 px-6 text-center">
				...
				<view class="flex space-x-4">
					<view class="bg-blue-500 text-white px-6 py-2 rounded-full text-sm font-medium" @click="goToSearch">重新搜索</view>
					<view class="bg-gray-100 text-gray-700 px-6 py-2 rounded-full text-sm font-medium" @click="goToHome">浏览热门</view>
				</view>
			</view>
		</scroll-view>
	</view>
	...
	methods: {
		...
		buildJQLSearchCondition() {
			...
			if (this.searchKeyword && this.searchKeyword.trim()) {
				...
				// 多字段模糊搜索
				const searchFields = [`${regex}.test(title)`, `${regex}.test(subtitle)`, `${regex}.test(destination)`];
