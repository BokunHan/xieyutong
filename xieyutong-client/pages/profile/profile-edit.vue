<template>
	<view class="bg-gray-50 min-h-screen">
			...
			<view v-if="!loading" class="bg-white rounded-xl mx-4 shadow-sm">
				...
				<view class="form-item">
					...
					<input 
						...
						class="form-input" 
						placeholder="请输入真实姓名" 
						v-model="realNameInfo.real_name"
					>
					<view class="form-right">
		...
		<view v-if="!loading" class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 safe-area-inset-bottom">
			<button class="save-btn w-full" @click="saveProfile">
				保存
			</button>
		</view>
	</view>
	...
	methods: {
		...
		async loadUserProfile() {
			...
			try {
				...
				const queryFields = '_id, username, avatar, mobile, birthday, realname_auth, register_date';
