	...
	<view class="page-container">
		...
		<view class="content-area" v-else-if="hasItinerary">
			...
			<view class="itinerary-header">
				...
				<view class="text-sm text-gray-500 mt-1">{{ itineraryData.dateRange }}</view>
		...
		<view class="content-area" v-else>
			<view class="empty-state">
				...
				<text class="text-gray-600">您目前没有进行中的旅行行程，快去探索心仪的旅行产品吧！</text>
...
export default {
	...
	methods: {
		 ...
		 */
		formatContent(rawText, options = {}) { // 修改点1: 增加 options 参数
			...
			try {
				...
				if (todayLocation) {
					weatherPromises.push(
						this.fetchLocationWeather(todayLocation, 'today')
					);
				}
				
				if (tomorrowLocation) {
					weatherPromises.push(
						this.fetchLocationWeather(tomorrowLocation, 'tomorrow')
					);
				}
			...
			try {
				...
				for (const activity of dayData.activities) {
					if (activity.location) {
						...
						// 提取城市名称
						const cityMatch = activity.location.match(/(北京|上海|广州|深圳|杭州|南京|苏州|成都|重庆|西安|武汉|长沙|郑州|济南|青岛|大连|沈阳|哈尔滨|长春|石家庄|太原|呼和浩特|银川|西宁|乌鲁木齐|拉萨|昆明|贵阳|南宁|海口|三亚|福州|厦门|南昌|合肥|兰州|林芝|日喀则|山南|那曲|阿里)/);
						if (cityMatch) {
					...
					// 从活动标题中提取位置
					if (activity.title) {
						const titleCityMatch = activity.title.match(/(北京|上海|广州|深圳|杭州|南京|苏州|成都|重庆|西安|武汉|长沙|郑州|济南|青岛|大连|沈阳|哈尔滨|长春|石家庄|太原|呼和浩特|银川|西宁|乌鲁木齐|拉萨|昆明|贵阳|南宁|海口|三亚|福州|厦门|南昌|合肥|兰州|林芝|日喀则|山南|那曲|阿里)/);
						if (titleCityMatch) {
		...
		extractLocationFromRoute(routeTitle) {
			...
			try {
				...
				// 2. 通用城市名称提取
				const cityMatch = routeTitle.match(/(北京|上海|广州|深圳|杭州|南京|苏州|成都|重庆|西安|武汉|长沙|郑州|济南|青岛|大连|沈阳|哈尔滨|长春|石家庄|太原|呼和浩特|银川|西宁|乌鲁木齐|拉萨|昆明|贵阳|南宁|海口|三亚|福州|厦门|南昌|合肥|兰州|林芝|日喀则|山南|那曲|阿里|昌都)/);
				if (cityMatch) {
