...
async function calculateUserConsumption(userId) {
	...
	if (result.data.length === 0) {
		...
		const allOrdersResult = await db.collection('a-orders')
			.field('user_id, status, _id, order_no')
			.limit(10)
			.get()
...
async function fetchMemberUpgradeRules() {
	...
	const result = await db.collection('a-system-configs')
		.where('config_key == "member_upgrade_rules" && status == "active"')
		.get()
	...
	if (result.data.length > 0) {
		console.log('[fetchMemberUpgradeRules] 使用数据库配置的升级规则:', result.data[0].config_value)
	...
	const defaultRules = {
		...
		gold: { required_consumption: 20000, discount_rate: 0.90 },
...
async function updateMemberLevel(userId, newLevel, consumptionData, upgradeRules) {
	...
	try {
		const updateResult = await db.collection('a-members')
			.where(`user_id == "${userId}"`)
			.update(updateData)
