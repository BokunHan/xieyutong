...
module.exports = {
	...
	async getUserProfile() {
		try {
			...
			// 使用 JQL 语法查询用户资料
			const userRes = await uniCloud.databaseForJQL({ // 获取JQL database引用，此处需要传入云对象的clientInfo
	...
	async updateUserProfile(profileData) {
		try {
			...
			// 使用 JQL 语法更新用户资料
			const updateRes = await uniCloud.databaseForJQL({ // 获取JQL database引用，此处需要传入云对象的clientInfo
				clientInfo: this.getClientInfo()
			})
				.collection('uni-id-users')
				.doc(uid)
